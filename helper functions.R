...
get_driver <- function() {
    ...
    ff_driver <- rsDriver(browser = "firefox",
                          chromever = "latest",
                          verbose = FALSE)    
    # return(ff_driver[["client"]])
}
...
prepare_webpage <- function(driver) {
    ...
    driver$navigate("https://wallstreetmillennial.com/login")
    Sys.sleep(3)
    # click_modal(driver = remote_driver)
    # Sys.sleep(3)
    login(driver = driver, login_details = login_details)
}
...
append_number_dups <- function(data) {
    ...
    for (field in fields) {
        ...
        field_ids <- tibble(field_matches = field_matches,
...
delete_old_files <- function(pattern, dir, step = 4) {
    ...
    file_dates_p1 <-
        files_raw %>% str_extract("\\d{4} \\d{2} \\d{2}")
    file_dates_p2 <-
        files_raw %>% str_extract("\\d{4}_\\d{2}_\\d{2}") %>%
        str_replace_all("_", " ")
    
    file_dates_p1[is.na(file_dates_p1)] <-
        file_dates_p2[!is.na(file_dates_p2)]
    ...
    files_to_del <-
        files_raw[which(file_dates_p1 %in% dates_to_del)] %>% sort()
...
rename_field_with_prior <-
    function(x,
        ...
        id_prior_field <-
            str_detect(x, pattern = paste0("^", required_prior_string, "$")) %>% which() %>% dplyr::first()
        if (is.na(id_prior_field))
            return(x)
        id_current_string <-
            str_detect(x, pattern = paste0("^", current_string, "$")) %>% which() %>% dplyr::first()
        if (is.na(id_current_string))
            return(x)
...
rename_field_conditionally <- function(x,
    ...
    if (is.null(current_string))
        stop("Please provide a current_string!")
    if (is.null(replacement))
        stop("Please provide a replacement_string!")
    ...
    id_current_string <-
        ...
        which() %>% first()
    if (is.na(id_current_string) ||
        length(id_current_string) == 0)
        return(x)
    
    if ((length(id_posterior_field) != 0) &
        (length(id_prior_field) != 0)) {
    ...
    if ((length(id_prior_field) != 0) &
        (length(id_posterior_field) != 0)) {
        if (!is.na(id_prior_field) & is.na(id_posterior_field) &
            (id_current_string > id_prior_field)) {
            x[id_current_string] <- replacement
        }
...
remove_leading_duplicates <- function(x, field = NULL) {
    ...
    if (is.null(field))
        stop("Provide a field")
    
    fields <- x %>% pull(field)
    
    dup_ids <-
        str_detect(fields, paste0("^", field, "$")) %>% which()
    if (length(dup_ids) %in% c(0, 1))
        return(x)
...
split_basic_and_diluted_eps <- function(x) {
    ...
    if (!"basic_and_diluted_eps" %in% fields)
        return(x)
    field_id <-
        str_detect(fields, "^basic_and_diluted_eps$") %>% which()
    ...
    cleaned_eps <-
        rbind(basic_and_diluted_eps, basic_and_diluted_eps) %>%
...
split_basic_and_diluted_shares <- function(x) {
    ...
    if (!"basic_and_diluted_shares" %in% fields)
        return(x)
    field_id <-
        str_detect(fields, "^basic_and_diluted_shares$") %>% which()
    if ((field_id %>% length()) > 1)
        return(x)
    ...
    cleaned_shares <-
        rbind(basic_and_diluted_shares, basic_and_diluted_shares) %>%
...
consolidate_dep_amor <- function(x) {
    ...
    if (length(dep_id) != 1 |
        length(amor_id) != 1)
        # If there is onlly one dep field and one amort field, then combine
        return(x)
    if (dep_id != amor_id) {
        dep_amor <-
...
consolidate_sga <- function(x) {
    ...
    if (str_detect(fields, "^selling_general_administrative$") %>% any())
        return(x)
    
    sell_id <- str_detect(fields, "^selling_expense$") %>% which()
    admin_id <-
        str_detect(fields, "^general_administrative_expense$") %>% which()
    
    if (length(sell_id) != 1 | length(admin_id) != 1)
        return(x)
    if (sell_id != admin_id) {
        sga <-
...
consolidate_selling_expense <- function(x) {
    ...
    if (str_detect(fields, "administrative") %>% any())
        return(x)
...
consolidate_general_administrative <- function(x) {
    ...
    if (str_detect(fields, "selling") %>% any())
        return(x)
...
# Handle modal
click_modal <- function(driver) {
    # driver <- remote_driver
    
    suppressMessages(tryCatch({
        modal_element <- driver$findElement(using = "xpath",
...
get_profile_data <- function(driver, url) {
    ...
    # Get company name
    name_element <- find_element_selector(driver, "#tableTitle")
